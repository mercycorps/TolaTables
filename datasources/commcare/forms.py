...
class CommCareProjectForm(forms.Form):
    ...
    TYPE_CHOICES = [('commcare_report', 'Report'), ('commcare_form', 'Form'),
                    ('cases', 'Cases')]
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['project'].widget = ListTextWidget(
            data_list=get_projects(user_id), name='projects')
    ...
    def clean(self):
        ...
        has_errors = False
        if download_type == 'commcare_report' and \
                commcare_report_name == 'default':
            self.add_error(
                'commcare_report_name', 'You must choose a report '
                'from the dropdown')
            has_errors = True
        if download_type == 'commcare_form' and \
                commcare_form_name == 'default':
            self.add_error(
                'commcare_form_name', 'You must choose a form '
                'from the dropdown')
            has_errors = True
        if int(silo) == 0:
...
class CommCareAuthForm(forms.Form):
    ...
    auth_token = forms.CharField(
        ...
        help_text=mark_safe(
            'This gives tola access to your CommCare reports. Your API '
            'Key can be found in your CommCare <a href="https://www.'
            'commcarehq.org/account/settings/" target="_blank">settings</a>'))
